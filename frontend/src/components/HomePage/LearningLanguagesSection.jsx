/**
 * LearningLanguageSection Component (Homepage)
 * FILE: src/components/HomePage/LearningLanguagesSection.jsx
 *
 * Changes from original:
 *  - "Smart Study" → "EduFlow" in heading and body copy
 *  - Get Started button now goes to /allCourses (not /signup)
 *    — allows already-logged-in users to browse directly
  ...
  return (
    <div className="py-16">
      ...
      <p className="text-center text-richblack-700 font-medium lg:w-[75%] mx-auto leading-6 text-base">
        EduFlow helps you track progress, compare performance, and plan your
        learning journey effectively. Explore structured courses, interactive
        lessons, and real-world skills development.
      </p>
      ...
      <div className="w-fit mx-auto mt-10">
        <button
          onClick={() => navigate(token ? "/allCourses" : "/signup")}
          className="bg-yellow-50 text-black px-6 py-3 rounded-lg font-bold hover:scale-95 hover:bg-richblack-800 hover:text-yellow-50 transition-all duration-200"
        >
          {token ? "Browse Courses" : "Get Started"}
        </button>
      </div>
    </div>
  );
