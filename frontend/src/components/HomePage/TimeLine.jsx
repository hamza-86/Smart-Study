/**
 * TimelineSection Component (Homepage)
 * FILE: src/components/HomePage/TimeLine.jsx
 *
 * Changes from original:
 *  - "Smart Study Timeline" alt text → "EduFlow Timeline"
 *  - All other layout and data unchanged
 */
  ...
  <div className="py-16">
    <div className="flex flex-col lg:flex-row gap-20 items-center">
      ...
      <div className="lg:w-[45%] flex flex-col gap-10">
        {timelineData.map((item, index) => (
          <div key={index} className="flex gap-6">
            <div className="w-[52px] h-[52px] bg-white rounded-full flex justify-center items-center shadow-[0_0_62px_0_#00000012] shrink-0">
              <img src={item.Logo} alt={item.Heading} />
            </div>
            <div>
              <h2 className="font-semibold text-[18px]">{item.Heading}</h2>
              <p className="text-base text-richblack-300">{item.Description}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="relative shadow-blue-200 shadow-[0px_0px_30px_0px]">
        <img
          src={TimeLineImage}
          alt="EduFlow Timeline"
          className="shadow-white shadow-[20px_20px_0px_0px] object-cover h-[400px] lg:h-auto rounded-lg"
        />
      </div>
